...
class TransformersSeqClassifierHandler(BaseHandler, ABC):
    ...
    def initialize(self, ctx):
        ...
        if self.setup_config["save_mode"] == "onnx":
            options = SessionOptions()
            self.session = InferenceSession("./model-quantized.onnx", options)
        elif self.setup_config["save_mode"] == "torchscript":
    ...
    def preprocess(self, requests):
        ...
        for idx, data in enumerate(requests):
            ...
            if self.setup_config["mode"] in {"sequence_classification", "token_classification", "text_generation"}:
                #inputs = self.tokenizer(input_text, max_length=int(max_length), pad_to_max_length=True, add_special_tokens=True, return_tensors='np')
                inputs = self.tokenizer(input_text, max_length=int(max_length), padding='max_length', add_special_tokens=True, return_tensors='pt')
    ...
    def inference(self, input_batch):
        ...
        if self.setup_config["mode"] == "sequence_classification":
            if self.setup_config["save_mode"] == "onnx":
                ...
                #onnx_inputs = {key: np.array([inputs[key]]) for key in input_keys}
                onnx_inputs = {'input_ids': input_ids_batch.numpy(), 'attention_mask': attention_mask_batch.numpy()}
                predictions = self.session.run(onnx_named_outputs, onnx_inputs)
            ...
            for i in range(num_rows):
                ...
                [score, y_hat] = out.max(1)
                score, y_hat = score.item(), y_hat.item()
                predicted_idx = str(y_hat)
                #inferences.append(self.mapping[predicted_idx])
                inferences.append([self.mapping[predicted_idx], score])
